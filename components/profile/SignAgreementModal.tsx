...
import { saveSignedAgreement } from '@/app/actions/rewards';
...
export default function SignAgreementModal({
  ...
  const companyName = profile?.company_name || profile?.full_name || 'Registered Seller';
  const legalName = profile?.legal_name || profile?.full_name || 'Authorized Representative';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="bg-zinc-800/95 backdrop-blur-lg border border-slate-200/50 rounded-3xl p-6 md:p-8 max-w-lg w-full shadow-2xl relative z-10 animate-fade-in-down max-h-[90vh] overflow-y-auto no-scrollbar space-y-6">
        <div className="text-center space-y-2">
          ...
          />
          <h3 className="text-lg font-black text-white tracking-tight uppercase">Sign Platform Vendor Agreement</h3>
          <p className="text-xs text-zinc-400 max-w-sm mx-auto leading-relaxed font-semibold">
            To continue selling on MechItAll, please review and agree to the latest marketplace vendor terms.
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="h-60 overflow-y-auto p-3 border border-zinc-700/60 rounded-lg bg-zinc-900/40 text-[10px] text-zinc-400 font-sans space-y-3 leading-relaxed no-scrollbar select-none">
          ...
          <div className="flex items-start gap-2 pt-1 pb-2">
            <input
              type="checkbox"
              id="agreeModalAgreement"
              checked={agreed}
              onChange={(e) => setAgreed(e.target.checked)}
              required
              className="mt-0.5 w-4 h-4 rounded text-[#007084] focus:ring-[#007084] bg-zinc-900 border-zinc-700 cursor-pointer"
            />
            <label htmlFor="agreeModalAgreement" className="text-[10px] text-zinc-300 font-bold select-none cursor-pointer leading-tight">
              I have read and agree to the MechItAll Vendor Agreement. I understand that my registered business name (<span className="text-[#00D0F5] font-extrabold">{companyName}</span>) and legal name (<span className="text-[#00D0F5] font-extrabold">{legalName}</span>) will be filled in the agreement. *
            </label>
          </div>
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  showToast(err.message || 'Failed to sign Vendor Agreement.', 'error');
                } finally {
                  setSigning(false);
                }
              }}
