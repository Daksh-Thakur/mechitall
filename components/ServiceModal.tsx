      ...
      <div className="bg-white border border-slate-border rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl relative z-10 flex flex-col md:flex-row h-[520px]">
        ...
        <div className="md:w-7/12 flex flex-col justify-between bg-white">
          <div className="p-6 pb-0 space-y-3">
            ...
            {/* Tabs */}
            <div className="flex gap-4 border-b border-slate-border text-xs font-bold">
              {(['details', 'reviews'] as const).map(t => (
                <button
                  key={t}
                  onClick={() => setActiveTab(t)}
                  className={`pb-2 border-b-2 transition-all cursor-pointer flex items-center gap-1.5 ${activeTab === t ? 'border-cobalt text-cobalt' : 'border-transparent text-slate-text-muted hover:text-slate-text-secondary'}`}
                >
              ...
              <div className="space-y-3">
                ...
                {showReviewForm && (
                  <form onSubmit={handleReviewSubmit} className="space-y-3 bg-slate-bg/40 border border-slate-border rounded-xl p-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button type="button" onClick={() => setShowReviewForm(false)} className="text-xs font-bold px-4 py-2 rounded-lg border border-slate-border text-slate-text-muted cursor-pointer">
                        Cancel
                      </button>
                    </div>
                  </form>
                ...
                {loadingReviews ? (
                  <div className="space-y-2 animate-pulse">
                    {[1, 2].map(n => <div key={n} className="h-16 bg-slate-bg/60 rounded-xl" />)}
                  </div>
                ) : reviews.length === 0 ? (
                  <div className="text-center py-6 border border-dashed border-slate-border rounded-xl">
                    <Star className="w-6 h-6 text-slate-text-muted/30 mx-auto mb-2" />
                    <p className="text-xs font-bold text-slate-text-primary">No reviews yet</p>
                    <p className="text-[10px] text-slate-text-muted">Be the first to review this service!</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {reviews.map(review => (
                      <div key={review.id} className="bg-white border border-slate-border rounded-xl p-3 space-y-2">
          ...
          {/* Footer */}
          <div className="pt-4 border-t border-slate-border p-6 flex items-center justify-between gap-4">
            <div>
